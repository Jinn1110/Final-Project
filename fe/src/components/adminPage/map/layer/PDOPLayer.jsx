...
const PDOPLayer = ({ tracks }) => {
  ...
  const pdop = tracks.dop?.pdop ?? null;
  ...
  const lng = loc.longitude ?? 10.58463218;

  const status = pdopStatus(pdop);
  const pdopFormatted =
    pdop !== null && pdop !== undefined ? pdop.toFixed(2) : "—";
          ...
          <div className="mb-3">
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                <div className="font-semibold text-lg">{pdopFormatted}</div>
              </div>
